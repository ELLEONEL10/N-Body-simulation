...
public:
    ...
    void runSimulation() {
        ...
        std::string filename = "parallel_output_" + getCurrentTime() + ".txt"; // Generate a filename with current time
        ...
        for (int step = 0; step < steps; ++step) {
            ...
            // Insert bodies into the quadtree in parallel
            #pragma omp parallel for // Distribute loop iterations among threads
            for (int i = 0; i < numBodies; ++i) {
                #pragma omp critical // Ensure that only one thread executes this block at a time
                insertBody(bodies[i]); // Insert the body into the quadtree
            }

            // Compute forces and update bodies in parallel
            #pragma omp parallel for // Distribute loop iterations among threads
            for (int i = 0; i < numBodies; ++i) {
                computeForce(bodies[i]); // Compute the gravitational force on the body
                updateBody(bodies[i]); // Update the body's position and velocity
            }
        }
